...
The BigQuery node *resource locators* (`projectId`/`datasetId` pickers) now hold `YOUR_GCP_PROJECT_ID` and `YOUR_BIGQUERY_DATASET` placeholders rather than a real project — re-pick your own in the n8n UI after import (see Security Notice below). Note that the SQL query nodes read `$env.BIGQUERY_PROJECT_ID` / `$env.BIGQUERY_DATASET`, so the env vars and the picker selections both need to point at the same place.
...
| BigQuery `projectId`/`datasetId` `__rl` objects | **Placeholders** | Now `YOUR_GCP_PROJECT_ID` / `YOUR_BIGQUERY_DATASET`, with the `cachedResultUrl` console links removed. Re-pick your own project and dataset in the n8n UI after import |
